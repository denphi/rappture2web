    ...
    getTraces(data, overrideLabel) {
        ...
        if (_curveType === 'mixed' && data._members) {
            return data._members.map((m) => {
                const isSep = m.label && m.label.toLowerCase().startsWith('separator');
                ...
                const lw = isSep ? 1 : (st.linewidth ? parseFloat(st.linewidth) : 2);
                return {
                    x: m.trace.x, y: m.trace.y, type: pt, mode: isSep ? 'lines' : pm, name: tLabel,
                    showlegend: !isSep,
                    ...(pt !== 'bar' ? { line: { width: lw, ...(traceColor ? { color: traceColor } : {}), ...(dash ? { dash } : {}) } } : {}),
                    ...(traceColor && pt === 'bar' ? { marker: { color: traceColor } } : {}),
                };
            });
        }
        ...
        sources.forEach(({ run, data }, si) => {
            ...
            if (_isMixedCmp && data._members) {
                data._members.forEach(m => {
                    const isSep = m.label && m.label.toLowerCase().startsWith('separator');
                    const mct = isSep ? 'line' : (m.curve_type || 'line').toLowerCase();
                    const mpt = mct === 'bar' ? 'bar' : 'scatter';
                    const mpm = mct === 'scatter' ? 'markers' : 'lines';
                    // Only push separator once (from first source to avoid duplicating the divider line)
                    if (isSep && !isTop) return;
                    traces.push({
